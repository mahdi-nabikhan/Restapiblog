...
class CustomTokenPairView(TokenObtainPairView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            response.set_cookie(
                "access", access_token, max_age=60, samesite="Lax", secure=False
            )
            response.set_cookie(
                "refresh",
                refresh_token,
            ...
            return Response(
                {"message": "failed on login"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            if not self.object.check_password(serializer.data.get("old_password")):
                return Response(
                    {"old password": "your password is wrong"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            self.object.set_password(serializer.data.get("new_password"))
            self.object.save()
            return Response(
                {"massage": "password changed successfully"}, status=status.HTTP_200_OK
            )
        else:
...
class ActivationApiView(APIView):
    ...
    def get(self, request, token, *args, **kwargs):
        try:
            ...
            return Response(
                data={"detail": "token has been expired"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        user.save()
        return Response(
            data={"detail": "user has been activated"}, status=status.HTTP_200_OK
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                send_activation_email.delay(user.email, user.email, access_token)
                return Response(
                    {"message": "Token resent to your email"}, status=status.HTTP_200_OK
                )
