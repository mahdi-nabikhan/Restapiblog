...
class TestPostViews:
    ...
    def test_patch_post_detail(self, api_client, django_user_model):
        ...
        post = Post.objects.create(
            title="Old", content="...", category=category, auther=user, status=True
        )
    ...
    def test_delete_post_detail(self, api_client, django_user_model):
        user = django_user_model.objects.create_user(
            email="delete@example.com", password="1234"
        )
        category = Category.objects.create(name="Cat")
        post = Post.objects.create(
            title="ToDelete", content="...", category=category, auther=user, status=True
        )
...
class TestCommentDetailAndDeleteAPI:
    ...
    def setup(self):
        ...
        self.comment = Comments.objects.create(
            user=self.user, post=self.post, content="Old comment"
        )
