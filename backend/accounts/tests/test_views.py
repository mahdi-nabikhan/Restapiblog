...
class TestCustomObtainToken(APITestCase):
    ...
    def test_login_nonexistent_email(self):
        data = {"email": "notfound@example.com", "password": "Whatever123"}
        response = self.client.post(self.url, data, format="json")
...
class TestActivationApiView:
    def setup_method(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            email="active@example.com", password="pass123", is_verified=False
        )
    ...
    def test_activate_user_invalid_token(self):
        url = reverse(
            "accounts:api-v1:activation-confirm", kwargs={"token": "invalidtoken"}
        )
        response = self.client.get(url)
        assert response.status_code == 400
